...
Señalar que la base de datos que se ha utilizado para realizar las pruebas de cada subsistema, está contenida en el archivo \textit{install-pruebas} del servidor front-end, adjunto con el código fuente.
...
Como se puede apreciar en dicho informe, en algunos paquetes se obtiene una cobertura que no está próxima al 90\%. Dichos paquetes son:
...
\begin{milista}
	...
	\item \textbf{Unidad de citación:} la cobertura disminuye en los siguientes paquetes:
		\begin{itemize}
			\item Paquete \textit{Presentación.auxiliar}: en este paquete se incluyen, enre otras, varias clases encargadas de gestionar eventos compartidos entre ventanas de la interfaz gráfica y clases encargadas de cambiar el aspecto en el que se muestran tablas y listas de elementos. Es por ello que la cobertura es menor, pues hay eventos de estas clases que no se hazn forzado a ejecutarse con las pruebas de interfaz. 
			\item Paquete \textit{Excepciones}: algunas excepciones incluyen dos métodos constructores, uno que no recibe mensaje y otro que sí lo recibe, para hacer más flexible el mensaje que se quiere mostrar cuando se produce dicha excepción. Sin embargo, no se han forzado situaciones en las pruebas para probar ambos constructores.
			...
			\item Paquete \textit{Dominio.control}: la cobertura disminuye debido a ciertas excepciones que no se fuerzan, como ya se comentó al inicio de esta sección. También, no se fuerza la desconexión del servidor ni sesiones duplicadas, por lo que esas instrucciones no se han probado.
			\item Paquete \textit{Comunicaciones}: la cobertura disminuye debido a ciertas excepciones que no se fuerzan, como ya se comentó al inicio de esta sección.
		\end{itemize}
...
\multicolumn{4}{c}{} \\
\hline
\multicolumn{4}{|>{\columncolor[rgb]{0.8, 0.8, 0.8}}c|}{\textbf{SUBSISTEMA: Unidad de citación (129 casos de prueba)}} \\
\hline
\multicolumn{4}{c}{} \\
\hline
\multicolumn{1}{|>{\columncolor[rgb]{0.8, 0.8, 0.8}}c|}{\textbf{PAQUETE}} & \multicolumn{1}{|>{\columncolor[rgb]{0.8, 0.8, 0.8}}p{3.7cm}|}{\textbf{COBERTURA(\%)}} & \multicolumn{1}{|>{\columncolor[rgb]{0.8, 0.8, 0.8}}p{3cm}|}{\textbf{INSTRUCCIONES CUBIERTAS}} & \multicolumn{1}{|>{\columncolor[rgb]{0.8, 0.8, 0.8}}p{3.7cm}|}{\textbf{INSTRUCCIONES TOTALES}} \\
\hline
\textbf{\textit{Unidad de Citación}} & \textbf{\textit{91.9}} & \textbf{\textit{28383}} & \textbf{\textit{31019}} \\
\hline
