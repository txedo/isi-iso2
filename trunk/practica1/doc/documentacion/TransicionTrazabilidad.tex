...
a partir de los diferentes diagramas del Proceso Unificado de Desarrollo, así
...
Según el diagrama de casos de uso del cliente, el caso de uso \textbf{Tramitar
cita} lo pueden realizar los citadores y, por herencia, también los
administradores del sistema. Sin embargo, como se explica en la descripción del
caso de uso, es necesario haber iniciado sesión primero, momento en el que se
determinará el rol del usuario.
...
diagrama \emph{Análisis - Iniciar sesión} del cliente, se define en la clase
\emph{JFLogin}.
...
el flujo indicado en el diagrama \emph{Análisis - Iniciar sesión} del cliente:
\begin{milista}
...
La identificación del usuario, que en el diagrama de análisis se muestra con
un único paso, se realiza en el código con dos debido a que la interfaz del
servidor front-end propuesta e implementada posee dos métodos diferentes para
identificarse y registrar el cliente en el sistema: \emph{identificar} y
\emph{registrar}. Las clases que intervienen en este proceso se pueden ver en
el diagrama \emph{Iniciar sesión} del cliente. \verb+<<<<<<<<<<<<<<<<<<<<<<<IMPORTANTE, ESTE DIAGRAMA NO ESTÁ HECHO TODAVÍA>>>>>>>>>>>>>>>>>>+
...
diagramas de análisis porque es un detalle de implementación del caso de uso
\textbf{Iniciar sesión}, que pretende hacer más extensible tanto el cliente
como el servidor, para que haya que hacer los menores cambios posibles si un
rol pasa a tener más o menos operaciones disponibles.
...
análisis del cliente se corresponde, en la implementación final, con un gran
número de clases JPanel, que dividen la funcionalidad del cliente en cinco
grandes grupos (gestión de beneficiarios, usuarios, citas, volantes y
...
consultar el diagrama \emph{Interfaz gráfica} del cliente. \verb+<<<<<<<<<<<<<IMPORTANTE, ESTE DIAGRAMA NO ESTÁ HECHO TODAVÍA>>>>>>>>>>>>>+
...
Por tanto, los primeros pasos del diagrama \emph{Análisis - Tramitar cita} del
cliente, en los que se obtiene y valida el NIF/NSS del beneficiario que quiere
pedir cita y se comprueba si existe realmente, los realiza el panel
\emph{JPBeneficiarioConsultar}, tal y como se puede ver en el diagrama de
secuencia \emph{Tramitar cita (tramitación correcta)} del cliente. El
siguiente fragmento de código muestra este proceso.
...
diagrama de secuencia \emph{Tramitar cita (tramitación correcta)} del cliente.
...
diagrama de análisis \emph{Análisis - Tramitar cita} y el diagrama de
secuencia \emph{Tramitar cita (tramitación correcta)} del cliente.
...
se puede ver en el diagrama de análisis \emph{Análisis - Tramitar cita}, el
...
Nótese que, en el diagrama de análisis citado, la interacción comienza con la
selección del beneficiario que quiere pedir cita, ya que es un paso
imprescindible para solicitar una cita. Sin embargo, como nos estamos centrando
en la trazabilidad del caso de uso \textbf{Tramitar cita}, no hemos comentado
en detalle cómo se gestiona la recuperación del beneficiario en el servidor, y
hemos pasado directamente al almacenamiento de la cita.

\texttt{\lstinputlisting[inputencoding=latin1]{codigos/TrazaCitaServidor.java}}

A continuación, conforme al diagrama de análisis, la clase
...
mostrada en el diagrama \emph{Tramitar cita (tramitación correcta)} del
servidor front-end para comprobar la validez de la cita y almacenarla en el
...
gestión de citas se pueden consultar en el diagrama de clases \emph{Gestor
Citas} del servidor front-end.
...
mensaje se puede ver en el diagrama \emph{Gestor Conexiones Log} del servidor.
