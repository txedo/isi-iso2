...
\subsection{Conexión de la aplicación Web con el servidor}

Para conectar la aplicación web con el servidor front-end del sistema de salud, se ha utilizado básicamente el mismo proxy que se empleó en la unidad de citación. Como se observa en el \diagrama{clases}{Conexión con el Servidor} (figura \ref{fig:diagramaServidor}), cada vez que una acción de \emph{Struts 2} o una página JSP de la web necesita solicitar una operación al servidor, se comunica con una clase singleton \emph{ServidorFrontend} que tiene asociada una instancia \emph{ProxyServidorFrontend}.

Este proxy, la primera vez que se utiliza, se conecta a través de \emph{RMI} a la interfaz \emph{IServidorFrontend} exportada por el servidor (por simplicidad, se supone que el servidor está ubicado en la misma máquina que la aplicación web). Desde ese momento, la aplicación web tendrá acceso completo a todas las operaciones de la interfaz del servidor, igual que si fuera un cliente más del sistema, con la única diferencia de que al no exportar la interfaz \emph{ICliente} no recibirá las notificaciones del servidor.

En resumen, como se observa en el \diagrama{despliegue}{Arquitectura del sistema} (figura \ref{fig:diagramaDespliegue}), tanto las unidades de citación como la aplicación web acceden a las funcionalidades del servidor front-end utilizando la misma interfaz. De esta forma, cuando desde la aplicación web se realicen cambios que afecten a los usuarios que hayan iniciado sesión en una unidad de citación, se refrescará la aplicación de escritorio igual que si esa operación se hubiera hecho desde otra unidad de citación.
...
\subsection{Cambios en la conexión con la base de datos}
...
A continuación se muestran los cuatro diagramas a los que se ha hecho referencia en los apartados anteriores. Los diagramas originales se pueden encontrar en el fichero de Visual Paradigm adjunto a este documento.

\imagenCompleta{imagenes/diagramaServidor.png}{Diagrama que muestra la conexión de la aplicación web con el servidor.}{fig:diagramaServidor}
