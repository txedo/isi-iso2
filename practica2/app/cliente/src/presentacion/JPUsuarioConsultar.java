...
public class JPUsuarioConsultar extends JPBase {
	...
	private void mostrarDatosUsuario(Usuario usuario) {
		...
		if(usuario.getRol().equals(Roles.Médico)) {
			rellenarTiposUsuario(new String[] { Roles.Médico.name() + " (" + ((Medico)usuario).getTipoMedico().getCategoria().name() + ")" });
	...
	private void btnGuardarActionPerformed(ActionEvent evt) {
		...
		try {
			...
			if(usuario.getRol() == Roles.Médico) {
				...
				if(cmbRol.getSelectedItem().toString().equals(Roles.Administrador.name())) {
					usuarioModif = new Administrador();
				} else if(cmbRol.getSelectedItem().toString().equals(Roles.Citador.name())) {
	...
	private void cmbRolesItemStateChanged(ItemEvent evt) {
		if(evt.getStateChange() == ItemEvent.SELECTED) {
			if(evt.getItem().equals(Roles.Médico.name())) {
				btnCalendario.setEnabled(true);
			} else {
				btnCalendario.setEnabled(false);
				periodos = new Vector<PeriodoTrabajo>();
